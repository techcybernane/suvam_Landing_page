...
export default function SiteSettingsPage() {
  ...
  const setPopup = (key) => (value) => setMeta((m) => ({ ...m, popup: { ...(m.popup || {}), [key]: value } }));
  ...
  return (
    <div>
      ...
      <div className="mt-6 admin-card space-y-4">
        <h2 className="font-bold text-ink">Popup Lead Form</h2>
        <p className="text-xs text-ink-soft">
          A modal that opens after a visitor has spent some time on the site. Submissions appear
          under <strong>Leads</strong> with the source “Popup Form”.
        </p>
        <ToggleField label="Show the popup on the public site" checked={popup.enabled !== false} onChange={setPopup("enabled")} />
        <div className="grid gap-4 sm:grid-cols-3">
          <TextField label="Open after (seconds)" value={popup.delaySeconds} onChange={setPopup("delaySeconds")} />
          <TextField label="Don't show again for (days)" value={popup.repeatAfterDays} onChange={setPopup("repeatAfterDays")} />
          <div className="self-end">
            <ToggleField
              label="Also open on exit intent"
              checked={popup.exitIntent !== false}
              onChange={setPopup("exitIntent")}
            />
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <TextField label="Eyebrow badge" value={popup.eyebrow} onChange={setPopup("eyebrow")} />
          <TextField label="Heading" value={popup.heading} onChange={setPopup("heading")} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <TextField label="Accent words (italic serif)" value={popup.accent} onChange={setPopup("accent")} />
          <TextField label="Submit button label" value={popup.submitLabel} onChange={setPopup("submitLabel")} />
        </div>
        <TextAreaField label="Body copy" value={popup.body} onChange={setPopup("body")} />
        <ImageField label="Side image" value={popup.image} onChange={setPopup("image")} />
        <TextField label="Service picker label" value={popup.serviceLabel} onChange={setPopup("serviceLabel")} />
        <StringListField label="Service options (chips)" items={popup.serviceOptions} onChange={setPopup("serviceOptions")} />
        <div className="grid gap-4 sm:grid-cols-2">
          <TextField label="Success heading" value={popup.successHeading} onChange={setPopup("successHeading")} />
          <TextField label="Small print under the button" value={popup.privacyNote} onChange={setPopup("privacyNote")} />
        </div>
        <TextAreaField label="Success message" value={popup.successBody} onChange={setPopup("successBody")} />
        <StringListField
          label="Hide the popup on these paths (e.g. /contact)"
          items={popup.hideOnPaths}
          onChange={setPopup("hideOnPaths")}
        />
      </div>
