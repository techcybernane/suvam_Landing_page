...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className="admin-surface min-h-screen bg-cream lg:flex lg:h-screen lg:overflow-hidden">
      <aside className="border-b border-line bg-white lg:flex lg:h-full lg:w-64 lg:shrink-0 lg:flex-col lg:overflow-y-auto lg:border-b-0 lg:border-r">
        <div className="flex items-center gap-2 px-6 py-6">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-forest text-lime-glow">
            <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none">
              <path d="M6 7l6 11 6-11" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </span>
          <div>
            <p className="text-sm font-extrabold text-ink">CYBERNANET</p>
            ...
            return (
              <Link
                ...
                className={`flex shrink-0 items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                  isActive ? "bg-forest text-lime-glow" : "text-ink-soft hover:bg-cream hover:text-ink"
                }`}
              >
        ...
        <div className="hidden border-t border-line px-3 py-4 lg:block">
          <Link href="/" target="_blank" className="flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium text-ink-soft hover:bg-cream hover:text-ink">
